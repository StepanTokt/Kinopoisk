import './authorizedPopUp.css';
import { useEffect, useState } from 'react'
import {useLoginUserMutation} from '../../logic/api/apiUsers';

export default function AuthorizedPopUp({active, setActive}) {
    const [username, setUsername] = useState('');
    const [password, setPassword] = useState('');
    const [loginUser, {isLoading, isFetching, isError}] = useLoginUserMutation();
    ...
    //чтобы убрать прокрутку модалки
    useEffect(() => {
    ...
    return (
        ...
        onClick={() => setActive(false)}>
            <div 
            className={active ? "pop_up modal_block active" : "pop_up modal_block"}
            onClick={(e) => e.stopPropagation()}>
                <div className="header_pop_up">
                    <a className="pop_up__title">Авторизация</a>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="legend">
                        <legend className="login_legend">Логин</legend>
                        <svg
                            width="5"
                            height="6"
                            viewBox="0 0 5 6"
                    ...
                    <div className="legend">
                        <legend className="password_legend">Пароль</legend>
                        <svg
                            width="5"
                            height="6"
                            viewBox="0 0 5 6"
                    ...
                    <div className="buttons">
                        <button type="submit" className="enter_button enter_pop_up" disabled={isLoading}>
                            Войти
                        </button>
                    ...
                    {isError && <p className="error_message">Неверный логин или пароль!</p>}
                </form>
            </div>    
        </div>
    )
